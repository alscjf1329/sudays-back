...
class TestPasswordUtil(unittest.TestCase):
    ...
    def test_verify_password(self):
        ...
        # 올바른 비밀번호 검증
        self.assertTrue(PasswordUtil.verify_password(password, hashed_password))
        
        # 잘못된 비밀번호 검증
        wrong_password = "wrong1234"
        self.assertFalse(PasswordUtil.verify_password(wrong_password, hashed_password))
        
    def test_different_salts(self):
        # 같은 비밀번호를 두 번 해싱했을 때 다른 해시가 생성되는지 확인
        password = "test1234"
        
        hashed1 = PasswordUtil.hash_password(password)
        ...
        self.assertTrue(PasswordUtil.verify_password(password, hashed1))
        self.assertTrue(PasswordUtil.verify_password(password, hashed2))
